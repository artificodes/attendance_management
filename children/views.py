...
def dashboard(request, *args, **kwargs):
    # groups = [['Melody',1],['Earth',2],['Grace',2],['Man',2],['Perfection',2],['New Beginning',1],
    # ['Fruit And Gifts of the Holy Spirit and Beatitudes',2],['Responsibility',2],
    # ['Prophetic Destiny',2],['Government',2],['Finishing',1]]
    # for index in range(3,len(groups)+3):
    #     createdgroup = chmodels.Group.objects.create(name=groups[index -3][0],age=index)
    #     createdgroup.refresh_from_db()
    #     for num in range(0,groups[index -3][1]):
    #         classroom = chmodels.ClassRoom.objects.create(name=str(groups[index-3][0]) + ' '+ str(num+1),capacity=100,arrangement_index=num)
    #         createdgroup.classrooms.add(classroom)
    #     # classroom = chmodels.ClassRoom.objects.create(name=str(groups[index-1]) + ' 2',capacity=100,arrangement_index=2)
    #     # createdgroup.classrooms.add(classroom)
    #     createdgroup.save()
    # existingmembers = mmodels.LraMembersBiodata.objects.all()
    # for existingmember in existingmembers:
    #     existingmember = mmodels.LraMembersBiodata.objects.filter(pk=existingmember.pk).values()[0]
    #     nums = '0123456789'
    #     tempnums = ''
    ...
    #     username = existingmember['surname'].lower()+existingmember['firstname'].lower()
    #     email =str(existingmember['email']).lower()
    #     formerrors = []
    #     try:
    ...
    #         pass
    #     try:
    #         user = User.objects.get(email=existingmember['email'])
    ...
    #     first_name =existingmember['firstname'],last_name= existingmember['surname'],
    #     email= email)
    #     exist = True
    #     while exist:
    #         chars = '0123456789' 
    ...
    #         except ObjectDoesNotExist:
    #             clonedmember = mmodels.Member.objects.create(user=user,memberid=userid,last_token=token,password=password)
    #             clonedmember.refresh_from_db()
    #             for attr, value in existingmember.items():
    #                 try:
    #                     setattr(clonedmember, attr, value)
    #                 except AttributeError:
    #                     continue
    #             clonedmember.save()
    #             existingdependants = chmodels.LraMembersChildren.objects.filter(parent_id=existingmember['member_id'])
    #             member = clonedmember
    #             if len(existingdependants) > 0:
    #                 parent = chmodels.Parent.objects.create(member=member)
    ...
    #                                 memberid=userid,date_of_birth = existingdependant.c_date_of_birth,
    #                                 full_name=existingdependant.c_firstname +' ' +existingdependant.c_surname)
    #                                 dependantexist = False
    
    #                             dependant = chmodels.Dependant.objects.create(parent=parent.member, first_name=existingdependant.c_firstname,
    #                             last_name=existingdependant.c_surname,date_of_birth = existingdependant.c_date_of_birth,member = dependantmember)
    ...
    if type(memberstatus) == bool:
        ...
        template_name = 'children/dashboard.html'
        member = allObject['member']
        ...
        # allarticles = chmodels.Article.objects.all()
        # articles = []
        # notifications = []
        ...
        # allannouncements = chmodels.Announcement.objects.all()
        # announcements = []
